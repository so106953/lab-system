...
# --- 3. 极致视觉定制 ---
st.set_page_config(page_title="UL登记系统", layout="centered")
        ...
        if res.data:
            ...
            for _, row in borrowed.iterrows():
                ...
                if now >= due_time:
                    ...
                    red_list.append(f"🔴 **{row['device_name']}** ({row['device_id']}) | 负责人: **{row['staff_id']}** | 已超期: **{overdue_str}**")
                elif now >= (due_time - timedelta(minutes=5)):
                    remain_str = format_time_diff(due_time - now)
                    yellow_list.append(f"🟡 **{row['device_name']}** ({row['device_id']}) | 负责人: **{row['staff_id']}** | 剩余: **{remain_str}**")

            if red_list:
                st.error("🚨 **设备超时未还警示名单**")
                for msg in red_list: st.markdown(f"<div style='padding:5px; border-left:5px solid red; background:rgba(255,0,0,0.05); margin-bottom:5px; font-size:13px;'>{msg}</div>", unsafe_allow_html=True)
            if yellow_list:
                st.warning("⏳ **即将超时提醒 (5分钟内)**")
                for msg in yellow_list: st.markdown(f"<div style='padding:5px; border-left:5px solid #FFCC00; background:rgba(255,204,0,0.05); margin-bottom:5px; font-size:13px;'>{msg}</div>", unsafe_allow_html=True)
    except: pass
    ...
    try:
        ...
        if res.data:
            ...
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                ...
                worksheet = writer.sheets['登记明细']

                # 定义填充颜色
                yellow_fill = PatternFill(start_color='FFFF00', end_color='FFFF00', fill_type='solid') # 亮黄
                green_fill = PatternFill(start_color='C6EFCE', end_color='C6EFCE', fill_type='solid')  # 浅绿

                # 遍历行，根据“类型”列（第3列）着色
                for row_idx, action in enumerate(df_display["类型"], start=2):
                    if action == "领用":
                        for cell in worksheet[row_idx]: cell.fill = yellow_fill
                    elif action == "归还":
                        for cell in worksheet[row_idx]: cell.fill = green_fill

            st.download_button(
                label=f"📥 导出已调色 Excel (.xlsx)",
                data=output.getvalue(),
                file_name=f"UL_{owning_group}_Colored_Records.xlsx",
